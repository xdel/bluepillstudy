...
VOID NTAPI HvmEventCallback (
  PCPU Cpu,//这个结构体的声明方式不懂，是按照什么来作依据的？在从svm-asm.asm的图中并未说明CPU中的内容是什么。
  PGUEST_REGS GuestRegs //这个数据结构在hvm.h中有说明，实际上对于amd64处理器来说，此处编译后是对一组64位寄存器的模拟，都放在了struts中。
)
{
  NTSTATUS Status;

  if (!Cpu || !GuestRegs)//验证参数
    return;

  // FIXME: This should be moved away from the HVM to VMX-specific code!!!
  if (Hvm->Architecture == ARCH_VMX)
    GuestRegs->rsp = VmxRead (GUEST_RSP);//RSP在这里被VmxRead方法赋值，不过这个方法只是与Intel架构相关，GUEST_RSP是指示存储在VMCS中的物理地址，从这里读到的值就是Guest的rsp的值。
										//GUEST_RSP = 0x0000681c,
  //嵌套guest的事件的处理
  if (Hvm->ArchIsNestedEvent (Cpu, GuestRegs)) {

    // it's an event of a nested guest
    Hvm->ArchDispatchNestedEvent (Cpu, GuestRegs);//怀疑此处是在中断处理，嵌套中断

    // FIXME: This should be moved away from the HVM to VMX-specific code!!!
	//恢复了原来GUEST_RSP所指地方的值，只有Intel要做这步而AMD不用，AMD中GuestRegs->rsp=0，因为从一开始也没保存，这就涉及到了上面的中断处理的上下文问题，按照这个表现
	//Intel中这个中断处理像是在Guest的上下文中，AMD则是在HVM的上下文中。AMD中这块也叫VMCB，Intel叫VMCS
    if (Hvm->Architecture == ARCH_VMX)
  ...
  Hvm->ArchDispatchEvent (Cpu, GuestRegs);//怀疑此处是在中断处理，非嵌套中断

  // FIXME: This should be moved away from the HVM to VMX-specific code!!!
  //跟上面一样，也是在恢复GUEST_RSP所指地方的值
  if (Hvm->Architecture == ARCH_VMX)
    VmxWrite (GUEST_RSP, GuestRegs->rsp);
