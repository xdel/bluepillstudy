...
)
{
  ...
  if (Cpu->ChickenQueueSize == 0) {
    DbgPrint("Chicken Queue Error: Attempt to dequeue element from empty queue!\n");
    return -1;
  }
...
)
{
  if (Cpu->ChickenQueueSize == CHICKEN_QUEUE_SZ) {
    DbgPrint("Chicken Queue Error: Attempt to enqueue element to already full queue!\n");
    return;
  }
...
{
  if (Cpu->ChickenQueueSize == 0) {
    DbgPrint("Chicken Queue Error: Attempt to get element from empty queue!\n");
    return -1;
  }
